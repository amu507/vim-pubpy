...
class VimEnviroment(object):
	...
	def confirm(self, msg, yes=False):
	...
	def choices(self,options,msg="please choose"):
		...
		try:
			input_str = int(vim.eval('inputlist(%s)'%choices))
		...
		try:
			...
			return self.choices(options,msg)
	...
	def run(self, name, *args):
		""" Run vim function. """
		sArgs = ", ".join([self.prepare_value(a) for a in args])
		sCMD="call %s(%s)" % (name,sArgs)
		vim.command(sCMD)

	def effqf(self,sqf):
		sqf=sqf.strip()
		self.run("ShowEffqf",sqf)

	def signeffqf(self,errs):
		self.run("SignEffqf",errs)

	def showqf(self,sqf):
		sPath=self.var("g:g_DataPath")+"\\tmpqf"
		f=file(sPath,"wb")
		f.write(sqf)
		f.close()
		self.exe("cgetfile "+sPath)

	def log(self,sLog):
		if not sLog.endswith("\n"):
			sLog+="\n"
		sPath=self.var("g:g_DataPath")+"\\log"
		f=file(sPath,"a")
		f.write(sLog)
		f.close()
		self.exe("cgetfile "+sPath)

	def exe(self,cmd,c=1):
		vim.command(cmd)
	...
	def prepare_value(self,value):
		...
		try:
			...
			try:
				value=json.dumps(value,encoding='gbk')
			except UnicodeDecodeError:
